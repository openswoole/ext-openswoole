--TEST--
openswoole_server: 10k connections
--SKIPIF--
<?php
...
$pm->childFunc = function () use ($pm) {
    ...
    $server->on('connect', function (OpenSwoole\Server $server, int $fd) {
        global $count;
        $count++;
        $server->send($fd, 'Hello Swoole!');
    });
    $server->on('receive', function (OpenSwoole\Server $serv, $fd, $rid, $data) { });
    $server->start();
};
$pm->childFirst();
$pm->run();
