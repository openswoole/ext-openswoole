--TEST--
openswoole_server: register sigint handler with base mode
--CONFLICTS--
all
--SKIPIF--
<?php
...
$pm->parentFunc = function ($pid) use ($pm) {
    usleep(10000);
    $result = OpenSwoole\Process::kill($pid, SIGINT);
    Assert::true($result);
    usleep(30000);
...
$pm->childFunc = function () use ($pm) {
    ...
    $server->on('workerStart', function (OpenSwoole\Server $server) use ($pm) {
