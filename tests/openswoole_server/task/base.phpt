--TEST--
openswoole_server/task: task & finish
--SKIPIF--
<?php
...
$simple_tcp_server = __DIR__ . "/../../include/api/openswoole_server/tcp_task_server.php";
...
go(function () use($port, $closeServer) {
    ...
    Assert::true($r);
    $cli->send("Test openswoole_server::task Interface.");
    $data = $cli->recv();
    Assert::same($data, "OK");
    $cli->close();
