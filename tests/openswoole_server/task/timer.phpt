...
$pm->childFunc = function () use ($pm) {
    ...
    $http->on('WorkerStart', function (Server $server, int $workerId) {
        if ($server->taskworker) {
            openswoole_timer_after(1, function () use ($server, $workerId) {
                var_dump("after1 : " . time());
            });
            // never callback
            openswoole_timer_after(10000, function () use ($server, $workerId) {
    ...
    $http->on('task', function (Server $server, Task $task) {
        var_dump('begin : ' . time());
        openswoole_timer_after(2000, function () use ($server, $task) {
            var_dump('end : ' . time());
