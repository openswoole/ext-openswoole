--TEST--
openswoole_server/task: taskwait in coroutine
--SKIPIF--
...
$pm->childFunc = function () use ($pm, $randoms) {
    ...
    $server->on('request', function (openswoole_http_request $request, openswoole_http_response $response) use ($server, $randoms) {
        $n = $request->get['n'];
    ...
    $server->on('task', function (openswoole_http_server $server, int $task_id, int $worker_id, string $n) use ($pm, $randoms) {
        return [$n, $randoms[$n]];
    });
    $server->start();
};
