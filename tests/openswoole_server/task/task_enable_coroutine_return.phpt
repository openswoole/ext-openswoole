--TEST--
openswoole_server/task: task enable coroutine with return value to finish
--SKIPIF--
<?php
...
$pm->childFunc = function () use ($pm) {
    $http = new openswoole_http_server('127.0.0.1', $pm->getFreePort(), SERVER_MODE_RANDOM);
    $http->set([
        'log_file' => '/dev/null',
        'task_worker_num' => 4,
    ...
    $http->on('request', function (openswoole_http_request $request, openswoole_http_response $response) use ($http) {
        ...
        $http->task($response->fd, -1, function ($server, $taskId, $data) {
            list($fd, $data) = $data;
            $response = openswoole_http_response::create($fd);
            $response->end($data);
        });
    });
    $http->on('task', function (openswoole_http_server $server, openswoole_server_task $task) {
        defer(function ($data) {
            if (!empty($data)) {
