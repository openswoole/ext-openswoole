--TEST--
openswoole_server/task: huge data
--SKIPIF--
<?php
...
$pm->childFunc = function () use ($pm) {
    ...
    $http->on('request', function (OpenSwoole\Http\Request $request, OpenSwoole\Http\Response $response) use ($http) {
    ...
    $http->on('task', function ($a, $b, $c, array $info) {
        $response = OpenSwoole\Http\Response::create($info['fd']);
