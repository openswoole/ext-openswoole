--TEST--
openswoole_server/object: status info
--SKIPIF--
...
use OpenSwoole\Server\StatusInfo;
...
{
    $cli = new Client(SWOOLE_SOCK_TCP, OPENSWOOLE_SOCK_SYNC);
...
Stack trace:
#0 [internal function]: {closure%S}(Object(OpenSwoole\Server), Object(OpenSwoole\Server\PipeMessage))
#1 %s(%d): OpenSwoole\Server->start()
#2 [internal function]: {closure%S}()
#3 %s(%d): call_user_func(Object(Closure))
#4 %s(%d): SwooleTest\ProcessManager->runChildFunc()
#5 [internal function]: SwooleTest\ProcessManager->%S{closure%S}(Object(OpenSwoole\Process))
#6 %s(%d): OpenSwoole\Process->start()
