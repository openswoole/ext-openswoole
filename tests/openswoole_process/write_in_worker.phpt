--TEST--
openswoole_process: write in worker
--SKIPIF--
...
$pm->childFunc = function () use ($pm, $counter) {
    ...
    $process = new \openswoole_process(function (openswoole_process $process) use ($serv, $counter) {
        if ($counter->get() != 1) {
            $counter->set(1);
            echo "process start\n";
            for ($i = 0; $i < 1024; $i++) {
    ...
    $serv->on("WorkerStart", function (\openswoole_server $serv) use ($process, $pm) {
        usleep(1);
        for ($i = 0; $i < 1024; $i++) {
