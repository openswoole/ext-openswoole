--TEST--
openswoole_http_client_coro: defer concurrency
--SKIPIF--
...
$pm->childFunc = function () use ($pm, $randomData) {
    $server = new openswoole_http_server('127.0.0.1', $pm->getFreePort(), SWOOLE_BASE);
    $server->set([
        'worker_num' => 1,
        'log_file' => '/dev/null'
    ]);
    ...
    $server->on('request', function (openswoole_http_request $request, openswoole_http_response $response) use ($pm, $server, $randomData) {
