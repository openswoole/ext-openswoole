--TEST--
openswoole_socket_coro: recvAll
--SKIPIF--
...
$pm->parentFunc = function ($pid) use ($pm) {
    go(function () use ($pm) {
         ...
         */
        //if (openswoole_string(CASE_LIST)->split(',')->contains(1, false)) {
            $conn->send("case-1");
            Co::usleep(1000);
            $header = $conn->recvAll(4);
            $len = unpack('Nlen', (string)$header)['len'];
         ...
         */
        //if (openswoole_string(CASE_LIST)->split(',')->contains(2, false)) {
            $conn->send("case-2");
            $header = $conn->recvAll(4);
            $len = unpack('Nlen', (string)$header)['len'];
            $body = $conn->recvAll($len);
         ...
         */
        //if (openswoole_string(CASE_LIST)->split(',')->contains(3, false)) {
            $conn->send("case-3");
            Co::usleep(1000);
            $header = $conn->recvAll(4);
            $len = unpack('Nlen', (string)$header)['len'];
         ...
         */
        //if (openswoole_string(CASE_LIST)->split(',')->contains(4, false)) {
            $conn->send("case-4");
            Co::usleep(1000);
            $header = $conn->recvAll(4);
            $len = unpack('Nlen', (string)$header)['len'];
...
$pm->childFunc = function () use ($pm) {
    ...
    $server->on('Receive', function (OpenSwoole\Server $server, $fd, $rid, $data) use ($pm) {
        if ($data == 'case-1') {
