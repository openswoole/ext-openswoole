--TEST--
openswoole_timer: call after in Task-Worker
--CONFLICTS--
all
--SKIPIF--
...
$pm->childFunc = function () use ($pm)
{
    ...
    $serv->on('task', function (openswoole_server $serv, $task_id, $worker_id, $data) {
        list($fd) = $data;
        openswoole_timer::after(500, function () use ($serv, $fd) {
            $serv->send($fd, "500\r\n\r\n");
            openswoole_timer::after(300, function () use ($serv, $fd) {
                $serv->send($fd, "800\r\n\r\n");
            });
        });
        openswoole_timer::after(1000, function () use ($serv, $fd) {
            $serv->send($fd, "1000[1]\r\n\r\n");
        });
        openswoole_timer::after(1000, function () use ($serv, $fd) {
            $serv->send($fd, "1000[2]\r\n\r\n");
        ...
        openswoole_timer::after(2000, function () use ($serv, $fd) {
            $serv->send($fd, "2000\r\n\r\n");
        });
    });
