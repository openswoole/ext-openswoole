--TEST--
openswoole_timer: timer in master
--CONFLICTS--
all
--SKIPIF--
...
$pm->childFunc = function () use ($pm) {
    ...
    $server->on('start', function (OpenSwoole\Server $server) use ($pm) {
        ...
        $id = OpenSwoole\Timer::tick(30, function () {
            file_put_contents(RES_FILE, "timer 1\n", FILE_APPEND);
        });
        OpenSwoole\Timer::after(90, function () use ($id, $server, $pm) {
            file_put_contents(RES_FILE, "timer 2\n", FILE_APPEND);
            OpenSwoole\Timer::clear($id);
            OpenSwoole\Timer::tick(10, function ($id) use ($server, $pm) {
                static $i = 0;
