--TEST--
openswoole_timer: timer in manager
--CONFLICTS--
all
--SKIPIF--
...
$pm->childFunc = function () use ($pm)
{
    ...
    $serv = new openswoole_server("0.0.0.0", $pm->getFreePort());
    ...
    $serv->on('managerStart', function ($serv) use ($pm) {
        ...
        $id = openswoole_timer_tick(300, function () {
            file_put_contents(RES_FILE, "timer 1\n", FILE_APPEND);
        });

        openswoole_timer_after(900, function () use ($id, $serv, $pm) {
            file_put_contents(RES_FILE, "timer 2\n", FILE_APPEND);
            openswoole_timer_clear($id);

            openswoole_timer_tick(200, function ($id) use ($serv, $pm) {
                static $i = 0;
    ...
    $serv->on('receive', function (openswoole_server $serv, $fd, $reactor_id, $data) {
