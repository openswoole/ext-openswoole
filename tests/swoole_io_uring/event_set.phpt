--TEST--
swoole_io_uring: event_set modifies callback with io_uring backend
--SKIPIF--
...
// Modify callback using swoole_event_set (triggers cancel + re-add in io_uring)
swoole_event_set($read_end, function ($fp) {
    $data = fread($fp, 8192);
    echo "updated: $data\n";
    swoole_event_del($fp);
    fclose($fp);
