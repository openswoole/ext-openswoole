--TEST--
swoole_io_uring: swoole_event_set called from within event handler
--SKIPIF--
...
swoole_event_add($read_end, function ($fp) use (&$call_count) {
    $call_count++;
    $data = fread($fp, 8192);
    ...
    if ($call_count === 1) {
        ...
        swoole_event_set($fp, function ($fp) use (&$call_count) {
            $call_count++;
            $data = fread($fp, 8192);
            echo "second: $data\n";
            swoole_event_del($fp);
...
swoole_event_defer(function () use ($write_end) {
