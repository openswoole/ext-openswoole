--TEST--
openswoole_feature/full_duplex: socket
--SKIPIF--
...
$pm->parentFunc = function ($pid) use ($pm) {
    ...
    $closer = go(function () {
        $closer = co::getCid();
        $timer = OpenSwoole\Timer::after(10 * 1000, function () use ($closer) {
            echo "TIMEOUT\n";
            co::resume($closer);
        });
        co::yield();
        if (OpenSwoole\Timer::exists($timer)) {
            OpenSwoole\Timer::clear($timer);
        }
        global $sockets;
