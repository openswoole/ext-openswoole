--TEST--
openswoole_feature/cross_close: http client closed by server
--SKIPIF--
...
$pm->childFunc = function () use ($pm) {
    $server = new OpenSwoole\Http\Server('127.0.0.1', $pm->getFreePort(), SERVER_MODE_RANDOM);
    $server->set(['log_file' => '/dev/null']);
    $server->on('workerStart', function () use ($pm) { $pm->wakeup(); });
    $server->on('request', function ($request, OpenSwoole\Http\Response $response) use ($server) {
        switch_process();
        echo "CLOSE\n";
