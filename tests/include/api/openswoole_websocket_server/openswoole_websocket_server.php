...
class WebSocketServer
{
    ...
    public function onStart(\openswoole_websocket_server $swooleServer)
    {
        debug_log("openswoole_server starting .....");
    }

    public function onShutdown(\openswoole_websocket_server $swooleServer)
    {
        debug_log("openswoole_server shutdown .....");
    }

    public function onWorkerStart(\openswoole_websocket_server $swooleServer, $workerId)
    {
        debug_log("worker #$workerId starting .....");
    }

    public function onWorkerStop(\openswoole_websocket_server $swooleServer, $workerId)
    {
        debug_log("worker #$workerId stopping ....");
    }

    public function onWorkerError(\openswoole_websocket_server $swooleServer, $workerId, $workerPid, $exitCode, $sigNo)
    {
    ...
    public function onOpen(openswoole_websocket_server $server, $request)
    {
        debug_log("{$request->fd} opened");
    }

    public function onMessage(openswoole_websocket_server $server, $frame)
    {
