...
class OpcodeServer
{
    ...
    public function start($lifetime = 1000)
    {
        ...
        /*
        $proc = new \openswoole_process(openswoole_function(\openswoole_process $proc) use($i) {
        ...
        $proc1 = new \openswoole_process(openswoole_function(\openswoole_process $proc) use($i) {
            var_dump($this->swooleServer->id);
            sleep(1000);
        });

        $proc2 = new \openswoole_process(openswoole_function(\openswoole_process $proc) {
            var_dump($this->swooleServer->id);
            sleep(1000);
        });
    ...
    public function onReceive(\openswoole_server $swooleServer, $fd, $fromReactorId, $recv)
    {
    ...
    public function onTask(\openswoole_server $swooleServer, $taskId, $fromWorkerId, $recv)
    {
    ...
    public function onFinish(\openswoole_server $swooleServer, $taskId, $recv)
    {
        $recv = json_decode($recv);
        Assert::same(json_last_error(), JSON_ERROR_NONE);
    ...
    public function onPipeMessage(\openswoole_server $swooleServer, $fromWorkerId, $recv)
    {
    ...
    public function onPacket(\openswoole_server $swooleServer, $data, array $clientInfo)
    {
