...
class HttpServer
{
    ...
    {
        if ($ssl) {
            $this->httpServ = new \openswoole_http_server($host, $port, SWOOLE_PROCESS, SWOOLE_SOCK_TCP | SWOOLE_SSL);
        } else {
            $this->httpServ = new \openswoole_http_server($host, $port, SWOOLE_PROCESS, SWOOLE_SOCK_TCP);
        }
    ...
    public function onStart(\openswoole_http_server $swooleServer)
    {
        debug_log("openswoole_server starting .....");
    }

    public function onShutdown(\openswoole_http_server $swooleServer)
    {
        debug_log("openswoole_server shutdown .....");
    }

    public function onWorkerStart(\openswoole_http_server $swooleServer, $workerId)
    {
        debug_log("worker #$workerId starting .....");
    }

    public function onWorkerStop(\openswoole_http_server $swooleServer, $workerId)
    {
        debug_log("worker #$workerId stopping ....");
    }

    public function onWorkerError(\openswoole_http_server $swooleServer, $workerId, $workerPid, $exitCode, $sigNo)
    {
        debug_log("worker error happening [workerId=$workerId, workerPid=$workerPid, exitCode=$exitCode, signalNo=$sigNo]...");
    }

    public function onReceive(\openswoole_http_server $swooleServer, $fd, $fromId, $data)
    {
    ...
    public function onRequest(\openswoole_http_request $request, \openswoole_http_response $response)
    {
        ...
        testSetCookie:
        {
            $name = "name";
            $value = "value";
            // $expire = $request->openswoole_server["request_time"] + 3600;
            $expire = 0;
            $path = "/";
            $domain = "";
            $secure = false;
        ...
        if ($uri === "/sleep") {
            openswoole_timer_after(1000, function() use($response) {
                $response->end();
            });
            return;
        }
        ...
        if ($uri === "/302") {
            $response->header("Location", "https://openswoole.com/");
            $response->status(302);
            $response->end();
            return;
        }
        ...
        if ($uri === "/chunked") {
            ...
            $timer = openswoole_timer_tick(200, function() use(&$timer, $writeChunk) {
                static $i = 0;
                $str = RandStr::gen($i++ % 40 + 1, RandStr::CHINESE) . "<br>";
                if ($writeChunk($str) === false) {
                    openswoole_timer_clear($timer);
                }
            });
            return;
        }
