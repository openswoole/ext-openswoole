--TEST--
openswoole_websocket_server: ping loop
--SKIPIF--
...
$pm->childFunc = function () use ($pm) {
    ...
    $serv->on('workerStart', function (openswoole_websocket_server $server) use ($pm) {
        ...
        $server->after(PING_LOOP * PING_INTERVAL, function () use ($pm, $server, $timer_id) {
            ...
            OpenSwoole\Coroutine::usleep(100000); // wait pong
            foreach ($server->connections as $fd) {
    ...
    $serv->on('message', function ($server, openswoole_websocket_frame $frame) use ($atomic) {
