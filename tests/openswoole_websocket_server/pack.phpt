--TEST--
openswoole_websocket_server: websocket frame pack/unpack
--SKIPIF--
...
use OpenSwoole\WebSocket\Frame as f;
use OpenSwoole\WebSocket\CloseFrame as cf;

for ($i = 1000; $i--;) {
    // generate some rand frames
    $opcode = mt_rand(WEBSOCKET_OPCODE_CONTINUATION, WEBSOCKET_OPCODE_PONG);
    $data = base64_encode(get_safe_random(mt_rand(1, 128000))) . 'EOL';
