--TEST--
openswoole_websocket_server: memory trace
--SKIPIF--
<?php
...
$pm->childFunc = function () use ($pm) {
    ...
    $server->on('workerStart', function (OpenSwoole\Websocket\Server $server, int $worker_id) use ($pm) {
        global $mem_records;
        phpt_echo("init\n");
        co::sleep(1);
        $pm->wakeup();
    ...
    $server->on('message', function (OpenSwoole\Websocket\Server $server, OpenSwoole\WebSocket\Frame $frame) { });
    $server->start();
};
$pm->childFirst();
$pm->run();
