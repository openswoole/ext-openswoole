--TEST--
openswoole_websocket_server: addlistener
--SKIPIF--
...
$pm->parentFunc = function (int $pid) use ($pm) {
    go(function () use ($pm) {
        $cli = new \OpenSwoole\Coroutine\Http\Client('127.0.0.1', 9506);
        $cli->set(['timeout' => 5]);
        $ret = $cli->upgrade('/');
        Assert::assert($ret);
...
$pm->childFunc = function () use ($pm) {
    ...
    $serv->on('message', function ($openswoole_server, $frame)
    {
        $openswoole_server->push($frame->fd, "Swoole: {$frame->data}");
    });
