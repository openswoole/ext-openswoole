--TEST--
openswoole_client_sync: recv in task worker
--SKIPIF--
...
$pm->childFunc = function () use ($pm) {
    $http = new openswoole_http_server('127.0.0.1', 0, SWOOLE_BASE);
    $http->set(['worker_num' => 1, 'task_worker_num'=>1, 'log_file' => '/dev/null']);
    $http->on('workerStart', function (openswoole_server $server, int $worker_id) use ($pm) {
    ...
    $http->on('Task', function(openswoole_server $serv, int $task_id, int $src_worker_id, $data) use ($pm) {
    ...
    $http->addProcess(new openswoole_process(function (openswoole_process $p) {
                }));
    $http->start();
};
$pm->childFirst();
