--TEST--
openswoole_client_sync: recv timeout
--SKIPIF--
...
$pm->parentFunc = function ($pid) use ($pm) {
    $client = new OpenSwoole\Client(SWOOLE_SOCK_TCP, OPENSWOOLE_SOCK_SYNC);
...
$pm->childFunc = function () use ($pm) {
    $serv = new \openswoole_server(TCP_SERVER_HOST, $pm->getFreePort(), SWOOLE_BASE, SWOOLE_SOCK_TCP);
    $serv->set([
        "worker_num" => 1,
        'log_file' => '/dev/null',
    ]);
