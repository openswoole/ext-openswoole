--TEST--
openswoole_http_server: http mixed server
--SKIPIF--
<?php
...
$pm->parentFunc = function ($pid) use ($pm, $tcp_options) {
    go(function () use ($pm, $tcp_options) {
        ...
        $http2_request = new openswoole_http2_request;
...
$pm->childFunc = function () use ($pm, $tcp_options) {
    $server = new openswoole_websocket_server('127.0.0.1', $pm->getFreePort(0), SWOOLE_BASE);
    $server->set([
        'worker_num' => 1,
        'log_file' => '/dev/null',
    ...
    // websocket
    $server->on('message', function (openswoole_websocket_server $server, openswoole_websocket_frame $frame) {
        $server->push($frame->fd, 'Hello ' . $frame->data);
    });
    // tcp
    $tcp_server = $server->listen('127.0.0.1', $pm->getFreePort(1), SWOOLE_TCP);
    $tcp_server->set($tcp_options);
    $tcp_server->on('receive', function (openswoole_server $server, int $fd, int $reactor_id, string $data) {
