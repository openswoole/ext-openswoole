--TEST--
openswoole_http_server: bug http_compression_level not work
--SKIPIF--
<?php
...
for ($level = MIN_COMPRESSION_LEVEL; $level <= MAX_COMPRESSION_LEVEL; $level++) {
    ...
    $pm->childFunc = function () use ($pm, $level, $randomBytes) {
        ...
        $http->on('request', function (OpenSwoole\Http\Request $request, OpenSwoole\Http\Response $response) use ($http, $randomBytes) {
