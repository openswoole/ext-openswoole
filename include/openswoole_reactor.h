...
class Reactor {
  public:
    ...
    ReactorHandler read_handler[OSW_MAX_FDTYPE] = {};
    ReactorHandler write_handler[OSW_MAX_FDTYPE] = {};
    ReactorHandler error_handler[OSW_MAX_FDTYPE] = {};
  ...
  public:
    Reactor(int max_event = OSW_REACTOR_MAXEVENTS, Type _type = TYPE_AUTO);
    ~Reactor();
    bool if_exit();
    ...
    inline int remove_write_event(network::Socket *_socket) {
        if (_socket->events & OSW_EVENT_READ) {
            _socket->events &= (~OSW_EVENT_WRITE);
            return set(_socket, _socket->events);
        } else {
            return del(_socket);
        }
    ...
    inline int add_read_event(network::Socket *_socket) {
        if (_socket->events & OSW_EVENT_WRITE) {
            ...
            return add(_socket, OSW_EVENT_READ);
        }
    }
    ...
    inline int add_write_event(network::Socket *_socket) {
        if (_socket->events & OSW_EVENT_READ) {
            ...
            return add(_socket, OSW_EVENT_WRITE);
        }
    }
    ...
    inline ReactorHandler get_error_handler(FdType fd_type) {
        ReactorHandler handler = get_handler(OSW_EVENT_ERROR, fd_type);
        // error callback is not set, try to use readable or writable callback
        if (handler == nullptr) {
            handler = get_handler(OSW_EVENT_READ, fd_type);
            if (handler == nullptr) {
                handler = get_handler(OSW_EVENT_WRITE, fd_type);
            }
        }
        return handler;
    }
    ...
    static FdType get_fd_type(int flags) {
        return (FdType) (flags & (~OSW_EVENT_READ) & (~OSW_EVENT_WRITE) & (~OSW_EVENT_ERROR) & (~OSW_EVENT_ONCE));
    }

    static bool isset_read_event(int events) {
        return (events < OSW_EVENT_DEAULT) || (events & OSW_EVENT_READ);
    }
    ...
    static bool isset_error_event(int events) {
        return events & OSW_EVENT_ERROR;
    }
};
}  // namespace openswoole

#define OSW_REACTOR_CONTINUE                                                                                            \
    if (reactor_->once) {                                                                                              \
        break;                                                                                                         \
    } else {                                                                                                           \
        continue;                                                                                                      \
