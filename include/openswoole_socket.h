...
#define openswoole_sendfile(out_fd, in_fd, offset, limit) sendfile(out_fd, in_fd, offset, limit)
#endif
...
struct Socket {
...
#ifdef TCP_CORK
        if (set_option(IPPROTO_TCP, TCP_CORK, nopush) == OSW_ERR) {
            return -1;
        } else {
            tcp_nopush = nopush;
            return 0;
    ...
    int set_tcp_nodelay(int nodelay = 1) {
        if (set_option(IPPROTO_TCP, TCP_NODELAY, nodelay) == OSW_ERR) {
            return -1;
        } else {
            tcp_nodelay = nodelay;
            return 0;
    ...
    bool check_liveness() {
        ...
        return !(retval == 0 || (retval < 0 && catch_error(errno) == OSW_CLOSE));
    }
    ...
    int listen(int backlog = 0) {
        return ::listen(fd, backlog <= 0 ? OSW_BACKLOG : backlog);
    }
    ...
    inline bool cork() {
        ...
        if (set_tcp_nopush(1) < 0) {
            openswoole_sys_warning("set_tcp_nopush(fd=%d, ON) failed", fd);
            return false;
        }
#endif
        // Need to turn off tcp nodelay when using nopush
        if (tcp_nodelay && set_tcp_nodelay(0) != 0) {
            openswoole_sys_warning("set_tcp_nodelay(fd=%d, OFF) failed", fd);
        }
        return true;
    }
    ...
    inline bool uncork() {
        ...
        if (set_tcp_nopush(0) < 0) {
            openswoole_sys_warning("set_tcp_nopush(fd=%d, OFF) failed", fd);
            return false;
        }
#endif
        // Restore tcp_nodelay setting
        if (enable_tcp_nodelay && tcp_nodelay == 0 && set_tcp_nodelay(1) != 0) {
            openswoole_sys_warning("set_tcp_nodelay(fd=%d, ON) failed", fd);
            return false;
        }
        return true;
    }
    ...
    bool isset_readable_event() {
        return events & OSW_EVENT_READ;
    }

    bool isset_writable_event() {
        return events & OSW_EVENT_WRITE;
    }
    ...
    static inline int is_dgram(SocketType type) {
        return (type == OSW_SOCK_UDP || type == OSW_SOCK_UDP6 || type == OSW_SOCK_UNIX_DGRAM);
    }

    static inline int is_stream(SocketType type) {
        return (type == OSW_SOCK_TCP || type == OSW_SOCK_TCP6 || type == OSW_SOCK_UNIX_STREAM);
    }

    bool is_stream() {
        return socket_type == OSW_SOCK_TCP || socket_type == OSW_SOCK_TCP6 || socket_type == OSW_SOCK_UNIX_STREAM;
    }

    bool is_dgram() {
        return socket_type == OSW_SOCK_UDP || socket_type == OSW_SOCK_UDP6 || socket_type == OSW_SOCK_UNIX_DGRAM;
    }

    bool is_inet4() {
        return socket_type == OSW_SOCK_TCP || socket_type == OSW_SOCK_UDP;
    }

    bool is_inet6() {
        return socket_type == OSW_SOCK_TCP6 || socket_type == OSW_SOCK_UDP6;
    }
    ...
    bool is_local() {
        return socket_type == OSW_SOCK_UNIX_STREAM || socket_type == OSW_SOCK_UNIX_DGRAM;
    }
    ...
    static inline SocketType convert_to_type(int domain, int type, int protocol = 0) {
        switch (domain) {
        case AF_INET:
            return type == SOCK_STREAM ? OSW_SOCK_TCP : OSW_SOCK_UDP;
        case AF_INET6:
            return type == SOCK_STREAM ? OSW_SOCK_TCP6 : OSW_SOCK_UDP6;
        case AF_UNIX:
            return type == SOCK_STREAM ? OSW_SOCK_UNIX_STREAM : OSW_SOCK_UNIX_DGRAM;
        default:
            return OSW_SOCK_TCP;
        }
    }
    ...
    static inline SocketType convert_to_type(std::string &host) {
        if (host.compare(0, 6, "unix:/", 0, 6) == 0) {
            ...
            return OSW_SOCK_UNIX_STREAM;
        } else if (host.find(':') != std::string::npos) {
            return OSW_SOCK_TCP6;
        } else {
            return OSW_SOCK_TCP;
        }
    }
