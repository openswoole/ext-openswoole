...
#define OSW_STRINGL(s) s->str, s->length
#define OSW_STRINGS(s) s->str, s->size
#define OSW_STRINGCVL(s) s->str + s->offset, s->length - s->offset
...
class String {
  private:
    void alloc(size_t _size, const Allocator *_allocator) {
        if (_allocator == nullptr) {
            _allocator = osw_std_allocator();
        }

        _size = OSW_MEM_ALIGNED_SIZE(_size);
        length = 0;
        size = _size;
        offset = 0;
  ...
  public:
    ...
    inline bool contains(const char *needle, size_t l_needle) {
        return openswoole_strnstr(str, length, needle, l_needle) != nullptr;
    }
    ...
    inline bool extend_align(size_t _new_size) {
        size_t align_size = OSW_MEM_ALIGNED_SIZE(size * 2);
    ...
    inline void write(off_t _offset, String *write_str) {
        ...
        if (new_length > size) {
            reserve(openswoole_size_align(new_length * 2, OpenSwooleG.pagesize));
        }
    ...
    inline size_t format(const char *format, Args... args) {
        size_t _size = osw_snprintf(nullptr, 0, format, args...);
        if (_size == 0) {
            return 0;
        }
        // store \0 terminator
        ...
        return (length = osw_snprintf(str, size, format, args...));
    }
