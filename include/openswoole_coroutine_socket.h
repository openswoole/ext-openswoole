...
class Socket {
  public:
    ...
    Socket(SocketType type = OSW_SOCK_TCP);
    ...
    static inline void init_reactor(Reactor *reactor) {
        reactor->set_handler(OSW_FD_CORO_SOCKET | OSW_EVENT_READ, readable_event_callback);
        reactor->set_handler(OSW_FD_CORO_SOCKET | OSW_EVENT_WRITE, writable_event_callback);
        reactor->set_handler(OSW_FD_CORO_SOCKET | OSW_EVENT_ERROR, error_event_callback);
    }
    ...
    inline bool has_bound(const EventType event = OSW_EVENT_RDWR) {
    ...
    inline long get_bound_cid(const EventType event = OSW_EVENT_RDWR) {
    ...
    inline void check_bound_co(const EventType event) {
        ...
        if (osw_unlikely(cid)) {
            openswoole_fatal_error(OSW_ERROR_CO_HAS_BEEN_BOUND,
    ...
    inline void set_err(int e) {
        errCode = errno = e;
        openswoole_set_last_error(errCode);
        errMsg = e ? openswoole_strerror(e) : "";
    }
    ...
    inline bool set_option(int level, int optname, int optval) {
        if (socket->set_option(level, optname, optval) < 0) {
            openswoole_sys_warning("setsockopt(%d, %d, %d, %d) failed", sock_fd, level, optname, optval);
            return false;
        }
        return true;
    }
    ...
    inline String *get_read_buffer() {
        if (osw_unlikely(!read_buffer)) {
            read_buffer = make_string(OSW_BUFFER_SIZE_BIG, buffer_allocator);
            if (!read_buffer) {
                throw std::bad_alloc();
            }
        }
    ...
    inline String *get_write_buffer() {
        if (osw_unlikely(!write_buffer)) {
            write_buffer = make_string(OSW_BUFFER_SIZE_BIG, buffer_allocator);
            if (!write_buffer) {
                throw std::bad_alloc();
            }
        }
  ...
  private:
...
#ifdef OSW_USE_OPENSSL
    EventType want_event = OSW_EVENT_NULL;
#endif
    ...
    size_t buffer_init_size = OSW_BUFFER_SIZE_BIG;
    ...
    inline void init_options() {
        if (type == OSW_SOCK_TCP || type == OSW_SOCK_TCP6) {
    ...
    inline bool is_available(const EventType event) {
        if (event != OSW_EVENT_NULL) {
            check_bound_co(event);
        }
        if (osw_unlikely(closed)) {
            set_err(ECONNRESET);
            return false;
        }
        return true;
      ...
      public:
        ...
        bool start() {
            if (timeout != 0 && !*timer_pp) {
                enabled = true;
                if (timeout > 0) {
                    *timer_pp = openswoole_timer_add((long) (timeout * 1000), false, callback, socket_);
    ...
    class TimeoutSetter {
      public:
        TimeoutSetter(Socket *socket, double timeout, const enum TimeoutType type)
            ...
            for (uint8_t i = 0; i < OSW_ARRAY_SIZE(timeout_type_list); i++) {
    ...
    class timeout_controller : public TimeoutSetter {
      public:
        ...
        inline bool has_timedout(const enum TimeoutType type) {
            OSW_ASSERT_1BYTE(type);
            if (timeout > 0) {
                if (osw_unlikely(startup_time == 0)) {
                    startup_time = microtime();
                } else {
                    double used_time = microtime() - startup_time;
                    if (osw_unlikely(timeout - used_time < OSW_TIMER_MIN_SEC)) {
...
openswoole::coroutine::Socket *openswoole_coroutine_get_socket_object(int sockfd);
