...
class Coroutine {
  public:
    ...
    typedef std::function<bool(openswoole::Coroutine *)> CancelFunc;
    ...
    inline long get_origin_cid() {
        return osw_likely(origin) ? origin->get_cid() : -1;
    }
    ...
    static inline long create(const CoroutineFunc &fn, void *args = nullptr) {
        ...
        try {
            ...
            openswoole_set_last_error(e.code().value());
            openswoole_warning("failed to create coroutine, Error: %s[%d]", e.what(), openswoole_get_last_error());
            return -1;
        }
#else
        return (new Coroutine(fn, args))->run();
    ...
    static inline Coroutine *get_current_safe() {
        if (osw_unlikely(!current)) {
            openswoole_fatal_error(OSW_ERROR_CO_OUT_OF_COROUTINE, "API must be called in the coroutine");
        }
        return current;
    }

    static inline void *get_current_task() {
        return osw_likely(current) ? current->get_task() : nullptr;
    }

    static inline long get_current_cid() {
        return osw_likely(current) ? current->get_cid() : -1;
    }

    static inline Coroutine *get_by_cid(long cid) {
        auto i = coroutines.find(cid);
        return osw_likely(i != coroutines.end()) ? i->second : nullptr;
    }

    static inline void *get_task_by_cid(long cid) {
        Coroutine *co = get_by_cid(cid);
        return osw_likely(co) ? co->get_task() : nullptr;
    }
    ...
    static inline void set_stack_size(size_t size) {
        stack_size = OSW_MEM_ALIGNED_SIZE_EX(OSW_MAX(MIN_STACK_SIZE, OSW_MIN(size, MAX_STACK_SIZE)), STACK_ALIGNED_SIZE);
    }
    ...
    static inline long get_elapsed(long cid) {
        ...
        return osw_likely(co) ? Timer::get_absolute_msec() - co->get_init_msec() : -1;
    }
  ...
  protected:
    ...
    Coroutine(const CoroutineFunc &fn, void *private_data) : ctx(stack_size, fn, private_data) {
        cid = ++last_cid;
        coroutines[cid] = this;
        if (osw_unlikely(count() > peak_num)) {
            peak_num = count();
        }
    }
    ...
    inline void check_end() {
        if (ctx.is_end()) {
            close();
        } else if (osw_unlikely(on_bailout)) {
            OSW_ASSERT(current == nullptr);
            on_bailout();
