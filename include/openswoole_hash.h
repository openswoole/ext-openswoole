 ...
 * jenkins
 */
static inline uint64_t openswoole_hash_jenkins(const char *key, size_t keylen) {
    uint64_t hashv;
 ...
 */
static inline uint32_t openswoole_hash_austin(const char *key, unsigned int keylen) {
    unsigned int h, k;
    h = 0 ^ keylen;
 ...
 */
static inline uint64_t openswoole_hash_php(const char *key, size_t len) {
    ulong_t hash = 5381;
