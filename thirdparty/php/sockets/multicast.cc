...
static int _php_mcast_join_leave(Socket *sock, int level, struct sockaddr *group, socklen_t group_len, unsigned int if_index, int join);
#ifdef HAS_MCAST_EXT
static int _php_mcast_source_op(Socket *sock, int level, struct sockaddr *group, socklen_t group_len, struct sockaddr *source, socklen_t source_len, unsigned int if_index, enum source_op sop);
#endif
...
static int php_get_if_index_from_array(const HashTable *ht, const char *key,
	Socket *sock, unsigned int *if_index)
{
	zval *val;
...
static int php_get_address_from_array(const HashTable *ht, const char *key,
	Socket *sock, php_sockaddr_storage *ss, socklen_t *ss_len)
{
	zval *val;
	zend_string *str;
...
static int php_do_mcast_opt(Socket *php_sock, int level, int optname, zval *arg4)
{
    HashTable *opt_ht;
    unsigned int if_index;
    int retval;
    int (*mcast_req_fun)(Socket *, int, struct sockaddr *, socklen_t, unsigned);
#ifdef HAS_MCAST_EXT
...
int php_do_setsockopt_ip_mcast(Socket *php_sock,
							   int level,
							   int optname,
							   zval *arg4)
{
...
int php_add4_to_if_index(struct in_addr *addr, Socket *php_sock, unsigned *if_index)
{
	...
	for (p = (char *) if_conf.ifc_buf;
		...
		if ((((struct sockaddr*)&cur_req->ifr_addr)->sa_family == AF_INET) &&
			...
			if (ioctl(php_sock->get_fd(), SIOCGIFINDEX, (char*)cur_req)
					== -1) {
