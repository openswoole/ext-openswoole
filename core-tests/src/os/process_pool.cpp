...
static void test_func(ProcessPool &pool) {
    ...
    int worker_id = -1;
    ASSERT_EQ(pool.dispatch_blocking(&data, &worker_id), OSW_OK);
...
TEST(process_pool, tcp) {
    ProcessPool pool{};
    ASSERT_EQ(pool.create(1, 0, OSW_IPC_SOCKET), OSW_OK);
    ASSERT_EQ(pool.listen(TEST_HOST, TEST_PORT, 128), OSW_OK);
...
TEST(process_pool, unix_sock) {
    ...
    ASSERT_EQ(pool.create(1, 0, OSW_IPC_SOCKET), OSW_OK);
    ASSERT_EQ(pool.listen(TEST_TMP_FILE, 128), OSW_OK);
...
TEST(process_pool, tcp_raw) {
    ...
    ASSERT_EQ(pool.dispatch_blocking(data.str, data.length), OSW_OK);
...
TEST(process_pool, msgqueue) {
    ProcessPool pool{};
    ASSERT_EQ(pool.create(1, 0x9501, OSW_IPC_MSGQUEUE), OSW_OK);
...
TEST(process_pool, shutdown) {
    ProcessPool pool{};
    int *shm_value = (int *) osw_mem_pool()->alloc(sizeof(int));
    ASSERT_EQ(pool.create(1, 0x9501, OSW_IPC_MSGQUEUE), OSW_OK);
