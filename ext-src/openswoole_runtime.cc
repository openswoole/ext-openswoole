...
#include "openswoole_runtime_arginfo.h"
...
using openswoole::coroutine::PollSocket;
using openswoole::coroutine::Socket;
using openswoole::coroutine::System;

OSW_EXTERN_C_BEGIN
static PHP_METHOD(openswoole_runtime, enableCoroutine);
static PHP_METHOD(openswoole_runtime, getHookFlags);
static PHP_METHOD(openswoole_runtime, setHookFlags);
static PHP_FUNCTION(openswoole_sleep);
static PHP_FUNCTION(openswoole_usleep);
static PHP_FUNCTION(openswoole_time_nanosleep);
static PHP_FUNCTION(openswoole_time_sleep_until);
static PHP_FUNCTION(openswoole_stream_select);
static PHP_FUNCTION(openswoole_stream_socket_pair);
static PHP_FUNCTION(openswoole_user_func_handler);
OSW_EXTERN_C_END
...
static zend_class_entry *openswoole_runtime_ce;
...
struct php_openswoole_netstream_data_t {
...
static const zend_function_entry openswoole_runtime_methods[] =
{
    PHP_ME(openswoole_runtime, enableCoroutine, arginfo_class_OpenSwoole_Runtime_enableCoroutine, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_runtime, getHookFlags, arginfo_class_OpenSwoole_Runtime_getHookFlags, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_runtime, setHookFlags, arginfo_class_OpenSwoole_Runtime_setHookFlags, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_FE_END
};
// clang-format on
...
#define OSW_HOOK_FUNC(f) hook_func(ZEND_STRL(#f), PHP_FN(openswoole_##f))
#define OSW_UNHOOK_FUNC(f) unhook_func(ZEND_STRL(#f))
#define OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(f)                                                                           \
    hook_func(ZEND_STRL(#f), PHP_FN(openswoole_native_##f), get_arginfo(ZEND_STRL("openswoole_native_" #f)))
...
void php_openswoole_runtime_minit(int module_number) {
    OSW_INIT_CLASS_ENTRY_BASE(
        openswoole_runtime, "OpenSwoole\\Runtime", "openswoole_runtime", nullptr, openswoole_runtime_methods, nullptr);
    OSW_SET_CLASS_CREATE(openswoole_runtime, osw_zend_create_object_deny);

    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_TCP"), PHPCoroutine::HOOK_TCP);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_UDP"), PHPCoroutine::HOOK_UDP);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_UNIX"), PHPCoroutine::HOOK_UNIX);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_UDG"), PHPCoroutine::HOOK_UDG);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_SSL"), PHPCoroutine::HOOK_SSL);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_TLS"), PHPCoroutine::HOOK_TLS);
    zend_declare_class_constant_long(
        openswoole_runtime_ce, ZEND_STRL("HOOK_STREAM_FUNCTION"), PHPCoroutine::HOOK_STREAM_FUNCTION);
    zend_declare_class_constant_long(
        openswoole_runtime_ce, ZEND_STRL("HOOK_STREAM_SELECT"), PHPCoroutine::HOOK_STREAM_FUNCTION);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_FILE"), PHPCoroutine::HOOK_FILE);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_STDIO"), PHPCoroutine::HOOK_STDIO);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_SLEEP"), PHPCoroutine::HOOK_SLEEP);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_PROC"), PHPCoroutine::HOOK_PROC);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_CURL"), PHPCoroutine::HOOK_CURL);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_NATIVE_CURL"), PHPCoroutine::HOOK_NATIVE_CURL);
    zend_declare_class_constant_long(
        openswoole_runtime_ce, ZEND_STRL("HOOK_BLOCKING_FUNCTION"), PHPCoroutine::HOOK_BLOCKING_FUNCTION);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_SOCKETS"), PHPCoroutine::HOOK_SOCKETS);
    zend_declare_class_constant_long(openswoole_runtime_ce, ZEND_STRL("HOOK_ALL"), PHPCoroutine::HOOK_ALL);

    // backward compatibility
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_TCP", PHPCoroutine::HOOK_TCP);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_UDP", PHPCoroutine::HOOK_UDP);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_UNIX", PHPCoroutine::HOOK_UNIX);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_UDG", PHPCoroutine::HOOK_UDG);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_SSL", PHPCoroutine::HOOK_SSL);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_TLS", PHPCoroutine::HOOK_TLS);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_STREAM_FUNCTION", PHPCoroutine::HOOK_STREAM_FUNCTION);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_STREAM_SELECT", PHPCoroutine::HOOK_STREAM_FUNCTION);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_FILE", PHPCoroutine::HOOK_FILE);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_STDIO", PHPCoroutine::HOOK_STDIO);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_SLEEP", PHPCoroutine::HOOK_SLEEP);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_PROC", PHPCoroutine::HOOK_PROC);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_CURL", PHPCoroutine::HOOK_CURL);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_NATIVE_CURL", PHPCoroutine::HOOK_NATIVE_CURL);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_BLOCKING_FUNCTION", PHPCoroutine::HOOK_BLOCKING_FUNCTION);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_SOCKETS", PHPCoroutine::HOOK_SOCKETS);
    OSW_REGISTER_LONG_CONSTANT("OPENSWOOLE_HOOK_ALL", PHPCoroutine::HOOK_ALL);

    // SWOOLE_HOOK_* backward compatibility aliases
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_TCP", PHPCoroutine::HOOK_TCP);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_UDP", PHPCoroutine::HOOK_UDP);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_UNIX", PHPCoroutine::HOOK_UNIX);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_UDG", PHPCoroutine::HOOK_UDG);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_SSL", PHPCoroutine::HOOK_SSL);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_TLS", PHPCoroutine::HOOK_TLS);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_STREAM_FUNCTION", PHPCoroutine::HOOK_STREAM_FUNCTION);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_STREAM_SELECT", PHPCoroutine::HOOK_STREAM_FUNCTION);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_FILE", PHPCoroutine::HOOK_FILE);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_STDIO", PHPCoroutine::HOOK_STDIO);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_SLEEP", PHPCoroutine::HOOK_SLEEP);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_PROC", PHPCoroutine::HOOK_PROC);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_CURL", PHPCoroutine::HOOK_CURL);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_NATIVE_CURL", PHPCoroutine::HOOK_NATIVE_CURL);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_BLOCKING_FUNCTION", PHPCoroutine::HOOK_BLOCKING_FUNCTION);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_SOCKETS", PHPCoroutine::HOOK_SOCKETS);
    OSW_REGISTER_LONG_CONSTANT("SWOOLE_HOOK_ALL", PHPCoroutine::HOOK_ALL);

#ifdef OSW_USE_CURL
    openswoole_native_curl_minit(module_number);
#endif
    openswoole_proc_open_init(module_number);
}
...
#ifdef OSW_USE_CURL
    openswoole_native_curl_mshutdown();
#endif
}
...
static php_stream_size_t socket_write(php_stream *stream, const char *buf, size_t count) {
    php_openswoole_netstream_data_t *abstract;
    Socket *sock;
    ssize_t didwrite = -1;

    abstract = (php_openswoole_netstream_data_t *) stream->abstract;
    ...
    if (didwrite < 0) {
        if (sock->errCode == ETIMEDOUT || sock->get_socket()->catch_error(sock->errCode) == OSW_WAIT) {
            didwrite = 0;
        } else {
            stream->eof = 1;
        }
...
static int socket_close(php_stream *stream, int close_handle) {
    php_openswoole_netstream_data_t *abstract = (php_openswoole_netstream_data_t *) stream->abstract;
    if (UNEXPECTED(!abstract)) {
        return FAILURE;
    }
...
#endif
        php_openswoole_netstream_data_t *abstract = (php_openswoole_netstream_data_t *) emalloc(sizeof(*abstract));
...
static inline int socket_xport_api(php_stream *stream, Socket *sock, php_stream_xport_param *xparam STREAMS_DC) {
    ...
    case STREAM_XPORT_OP_SEND:
        if ((xparam->inputs.flags & STREAM_OOB) == STREAM_OOB) {
            php_openswoole_error(E_WARNING, "STREAM_OOB flags is not supports");
            xparam->outputs.returncode = -1;
            break;
        }
        xparam->outputs.returncode =
    ...
    default:
#ifdef OSW_DEBUG
        php_openswoole_fatal_error(E_WARNING, "socket_xport_api: unsupported option %d", xparam->op);
#endif
        break;
    }
...
static int socket_set_option(php_stream *stream, int option, int value, void *ptrparam) {
    ...
    default:
#ifdef OSW_DEBUG
        php_openswoole_fatal_error(E_WARNING, "socket_set_option: unsupported option %d with value %d", option, value);
#endif
        break;
    }
    return PHP_STREAM_OPTION_RETURN_OK;
...
static bool socket_ssl_set_options(Socket *sock, php_stream_context *context) {
    if (context && ZVAL_IS_ARRAY(&context->options)) {
        ...
        if (sock->ssl_is_enable() && php_openswoole_array_get_value(Z_ARRVAL_P(&context->options), "ssl", ztmp) &&
            ZVAL_IS_ARRAY(ztmp)) {
            zval zalias;
            array_init(&zalias);
            ...
            auto add_alias = [&zalias, options](const char *name, const char *alias) {
                zval *ztmp;
                if (php_openswoole_array_get_value_ex(options, name, ztmp)) {
            ...
            php_openswoole_socket_set_ssl(sock, &zalias);
            if (!sock->ssl_check_context()) {
                return false;
            }
            zval_dtor(&zalias);
...
static php_stream *socket_create(const char *proto,
    ...
    php_openswoole_netstream_data_t *abstract = nullptr;
    Socket *sock;

    Coroutine::get_current_safe();

    if (OSW_STREQ(proto, protolen, "tcp")) {
    _tcp:
        sock = new Socket(resourcename[0] == '[' ? OSW_SOCK_TCP6 : OSW_SOCK_TCP);
    } else if (OSW_STREQ(proto, protolen, "ssl") || OSW_STREQ(proto, protolen, "tls")) {
#ifdef OSW_USE_OPENSSL
        sock = new Socket(resourcename[0] == '[' ? OSW_SOCK_TCP6 : OSW_SOCK_TCP);
        sock->enable_ssl_encrypt();
#else
        ...
        return nullptr;
#endif
    } else if (OSW_STREQ(proto, protolen, "unix")) {
        sock = new Socket(OSW_SOCK_UNIX_STREAM);
    } else if (OSW_STREQ(proto, protolen, "udp")) {
        sock = new Socket(OSW_SOCK_UDP);
    } else if (OSW_STREQ(proto, protolen, "udg")) {
        sock = new Socket(OSW_SOCK_UNIX_DGRAM);
    } else {
        /* abort? */
        goto _tcp;
    }
    ...
    abstract = (php_openswoole_netstream_data_t *) ecalloc(1, sizeof(*abstract));
...
static ZEND_FUNCTION(openswoole_display_disabled_function) {
...
static bool disable_func(const char *name, size_t l_name) {
    ...
    if (rf) {
        rf->function->internal_function.handler = ZEND_FN(openswoole_display_disabled_function);
        return true;
    }
    ...
    zf->internal_function.handler = ZEND_FN(openswoole_display_disabled_function);
...
bool PHPCoroutine::enable_hook(uint32_t flags) {
    ...
    if (flags & PHPCoroutine::HOOK_STREAM_FUNCTION) {
        if (!(runtime_hook_flags & PHPCoroutine::HOOK_STREAM_FUNCTION)) {
            OSW_HOOK_FUNC(stream_select);
            OSW_HOOK_FUNC(stream_socket_pair);
        }
    } else {
        if (runtime_hook_flags & PHPCoroutine::HOOK_STREAM_FUNCTION) {
            OSW_UNHOOK_FUNC(stream_select);
            OSW_UNHOOK_FUNC(stream_socket_pair);
        }
    }
    // file
    if (flags & PHPCoroutine::HOOK_FILE) {
        if (!(runtime_hook_flags & PHPCoroutine::HOOK_FILE)) {
            memcpy((void *) &php_plain_files_wrapper, &osw_php_plain_files_wrapper, sizeof(php_plain_files_wrapper));
        }
    } else {
        if (runtime_hook_flags & PHPCoroutine::HOOK_FILE) {
    ...
    if (flags & PHPCoroutine::HOOK_STDIO) {
        if (!(runtime_hook_flags & PHPCoroutine::HOOK_STDIO)) {
            memcpy((void *) &php_stream_stdio_ops, &osw_php_stream_stdio_ops, sizeof(php_stream_stdio_ops));
        }
    } else {
        if (runtime_hook_flags & PHPCoroutine::HOOK_STDIO) {
    ...
    if (flags & PHPCoroutine::HOOK_SLEEP) {
        if (!(runtime_hook_flags & PHPCoroutine::HOOK_SLEEP)) {
            OSW_HOOK_FUNC(sleep);
            OSW_HOOK_FUNC(usleep);
            OSW_HOOK_FUNC(time_nanosleep);
            OSW_HOOK_FUNC(time_sleep_until);
        }
    } else {
        if (runtime_hook_flags & PHPCoroutine::HOOK_SLEEP) {
            OSW_UNHOOK_FUNC(sleep);
            OSW_UNHOOK_FUNC(usleep);
            OSW_UNHOOK_FUNC(time_nanosleep);
            OSW_UNHOOK_FUNC(time_sleep_until);
        }
    }
    // proc_open
    if (flags & PHPCoroutine::HOOK_PROC) {
        if (!(runtime_hook_flags & PHPCoroutine::HOOK_PROC)) {
            OSW_HOOK_FUNC(proc_open);
            OSW_HOOK_FUNC(proc_close);
            OSW_HOOK_FUNC(proc_get_status);
            OSW_HOOK_FUNC(proc_terminate);
        }
    } else {
        if (runtime_hook_flags & PHPCoroutine::HOOK_PROC) {
            OSW_UNHOOK_FUNC(proc_open);
            OSW_UNHOOK_FUNC(proc_close);
            OSW_UNHOOK_FUNC(proc_get_status);
            OSW_UNHOOK_FUNC(proc_terminate);
        }
    }
    // blocking function
        ...
        if (is_core_loaded()) {
            if (flags & PHPCoroutine::HOOK_BLOCKING_FUNCTION) {
                if (!(runtime_hook_flags & PHPCoroutine::HOOK_BLOCKING_FUNCTION)) {
                    hook_func(ZEND_STRL("gethostbyname"), PHP_FN(openswoole_coroutine_gethostbyname));
                    hook_func(ZEND_STRL("exec"));
                    hook_func(ZEND_STRL("shell_exec"));
                }
            } else {
                if (runtime_hook_flags & PHPCoroutine::HOOK_BLOCKING_FUNCTION) {
                    OSW_UNHOOK_FUNC(gethostbyname);
                    OSW_UNHOOK_FUNC(exec);
                    OSW_UNHOOK_FUNC(shell_exec);
                }
            }
        } else {
        ...
        if (is_core_loaded()) {
            if (flags & PHPCoroutine::HOOK_SOCKETS) {
                if (!(runtime_hook_flags & PHPCoroutine::HOOK_SOCKETS)) {
                    ...
                    hook_func(ZEND_STRL("socket_create_pair"), PHP_FN(openswoole_coroutine_socketpair));
                ...
                if (runtime_hook_flags & PHPCoroutine::HOOK_BLOCKING_FUNCTION) {
                    OSW_UNHOOK_FUNC(socket_create);
                    OSW_UNHOOK_FUNC(socket_create_listen);
                    OSW_UNHOOK_FUNC(socket_create_pair);
                    OSW_UNHOOK_FUNC(socket_connect);
                    OSW_UNHOOK_FUNC(socket_write);
                    OSW_UNHOOK_FUNC(socket_read);
                    OSW_UNHOOK_FUNC(socket_send);
                    OSW_UNHOOK_FUNC(socket_recv);
                    OSW_UNHOOK_FUNC(socket_sendto);
                    OSW_UNHOOK_FUNC(socket_recvfrom);
                    OSW_UNHOOK_FUNC(socket_bind);
                    OSW_UNHOOK_FUNC(socket_listen);
                    OSW_UNHOOK_FUNC(socket_accept);
                    OSW_UNHOOK_FUNC(socket_getpeername);
                    OSW_UNHOOK_FUNC(socket_getsockname);
                    OSW_UNHOOK_FUNC(socket_getopt);
                    OSW_UNHOOK_FUNC(socket_get_option);
                    OSW_UNHOOK_FUNC(socket_setopt);
                    OSW_UNHOOK_FUNC(socket_set_option);
                    OSW_UNHOOK_FUNC(socket_set_block);
                    OSW_UNHOOK_FUNC(socket_set_nonblock);
                    OSW_UNHOOK_FUNC(socket_shutdown);
                    OSW_UNHOOK_FUNC(socket_close);
                    OSW_UNHOOK_FUNC(socket_clear_error);
                    OSW_UNHOOK_FUNC(socket_last_error);
                }
            }
    ...
    if (flags & PHPCoroutine::HOOK_NATIVE_CURL) {
        ...
        //     php_openswoole_fatal_error(E_WARNING, "cannot enable both hooks HOOK_NATIVE_CURL and HOOK_CURL at same
        //     time"); flags ^= PHPCoroutine::HOOK_CURL;
        // }
        if (!(runtime_hook_flags & PHPCoroutine::HOOK_NATIVE_CURL)) {
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_close);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_copy_handle);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_errno);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_error);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_exec);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_getinfo);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_init);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_setopt);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_setopt_array);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_reset);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_pause);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_escape);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_unescape);

            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_init);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_add_handle);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_exec);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_errno);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_select);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_setopt);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_getcontent);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_info_read);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_remove_handle);
            OSW_HOOK_NATIVE_FUNC_WITH_ARG_INFO(curl_multi_close);
        }
    } else {
        if (runtime_hook_flags & PHPCoroutine::HOOK_NATIVE_CURL) {
            OSW_UNHOOK_FUNC(curl_close);
            OSW_UNHOOK_FUNC(curl_copy_handle);
            OSW_UNHOOK_FUNC(curl_errno);
            OSW_UNHOOK_FUNC(curl_error);
            OSW_UNHOOK_FUNC(curl_exec);
            OSW_UNHOOK_FUNC(curl_getinfo);
            OSW_UNHOOK_FUNC(curl_init);
            OSW_UNHOOK_FUNC(curl_setopt);
            OSW_UNHOOK_FUNC(curl_setopt_array);
            OSW_UNHOOK_FUNC(curl_reset);
            OSW_UNHOOK_FUNC(curl_pause);
            OSW_UNHOOK_FUNC(curl_escape);
            OSW_UNHOOK_FUNC(curl_unescape);

            OSW_UNHOOK_FUNC(curl_multi_init);
            OSW_UNHOOK_FUNC(curl_multi_add_handle);
            OSW_UNHOOK_FUNC(curl_multi_exec);
            OSW_UNHOOK_FUNC(curl_multi_errno);
            OSW_UNHOOK_FUNC(curl_multi_select);
            OSW_UNHOOK_FUNC(curl_multi_setopt);
            OSW_UNHOOK_FUNC(curl_multi_getcontent);
            OSW_UNHOOK_FUNC(curl_multi_info_read);
            OSW_UNHOOK_FUNC(curl_multi_remove_handle);
            OSW_UNHOOK_FUNC(curl_multi_close);
        }
    }
#endif
...
static PHP_METHOD(openswoole_runtime, enableCoroutine) {
    if (!OPENSWOOLE_G(cli)) {
        php_openswoole_fatal_error(E_ERROR, "must be used in PHP CLI mode");
        RETURN_FALSE;
    }
    zval *zflags = nullptr;
    ...
    if (zflags) {
        if (Z_TYPE_P(zflags) == IS_LONG) {
            flags = OSW_MAX(0, Z_LVAL_P(zflags));
...
static PHP_FUNCTION(openswoole_time_nanosleep) {
    ...
    if (Coroutine::get_current()) {
        ...
        if (nanosleep(&php_req, &php_rem) == 0) {
            ...
            php_openswoole_error(E_WARNING, "nanoseconds was not in the range 0 to 999 999 999 or seconds was negative");
        }
    }
    RETURN_TRUE;
}
...
static void stream_array_to_fd_set(zval *stream_array, std::unordered_map<int, PollSocket> &fds, int event) {
    ...
    ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(stream_array), index, key, elem) {
        ZVAL_DEREF(elem);
        sock = php_openswoole_convert_to_fd(elem);
        if (sock < 0) {
            continue;
        }
...
static PHP_FUNCTION(openswoole_stream_select) {
    ...
    if (r_array != nullptr) {
        stream_array_to_fd_set(r_array, fds, OSW_EVENT_READ);
    }

    if (w_array != nullptr) {
        stream_array_to_fd_set(w_array, fds, OSW_EVENT_WRITE);
    }

    if (e_array != nullptr) {
        stream_array_to_fd_set(e_array, fds, OSW_EVENT_ERROR);
    }
    ...
    for (auto &i : fds) {
        ...
        OSW_ASSERT((revents & (~(OSW_EVENT_READ | OSW_EVENT_WRITE | OSW_EVENT_ERROR))) == 0);
        if (revents > 0) {
            if ((revents & OSW_EVENT_READ) && r_array) {
                kv->add_to(r_array);
            }
            if ((revents & OSW_EVENT_WRITE) && w_array) {
                kv->add_to(w_array);
            }
            if ((revents & OSW_EVENT_ERROR) && e_array) {
                kv->add_to(e_array);
            }
            retval++;
        }
...
static void hook_func(const char *name, size_t l_name, zif_handler handler, zend_internal_arg_info *arg_info) {
     ...
     */
    if (handler == nullptr) {
        handler = PHP_FN(openswoole_user_func_handler);
        use_php_func = true;
    }
    if (rf) {
    ...
    if (use_php_func) {
        ...
        if (!osw_zend_is_callable_ex(&rf->name, nullptr, 0, &func_name, nullptr, func_cache, nullptr)) {
            php_openswoole_fatal_error(
...
php_stream *php_openswoole_create_stream_from_socket(php_socket_t _fd, int domain, int type, int protocol STREAMS_DC) {
    ...
    php_openswoole_netstream_data_t *abstract = (php_openswoole_netstream_data_t *) ecalloc(1, sizeof(*abstract));
...
php_stream_ops *php_openswoole_get_ori_php_stream_stdio_ops() {
...
static PHP_FUNCTION(openswoole_stream_socket_pair) {
    ...
    s1 = php_openswoole_create_stream_from_socket(pair[0], domain, type, protocol STREAMS_CC);
    s2 = php_openswoole_create_stream_from_socket(pair[1], domain, type, protocol STREAMS_CC);
