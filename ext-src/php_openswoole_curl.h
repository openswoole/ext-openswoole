...
class Multi {
    ...
    void add_timer(long timeout_ms) {
        if (timer && openswoole_timer_is_available()) {
            openswoole_timer_del(timer);
        }
        timeout_ms_ = timeout_ms;
        timer = openswoole_timer_add(timeout_ms, false, [this](Timer *timer, TimerNode *tnode) {
            this->timer = nullptr;
            callback(nullptr, 0);
        });
    }
  ...
  public:
    ...
    int get_event(int action) {
        return action == CURL_POLL_IN ? OSW_EVENT_READ : OSW_EVENT_WRITE;
    }

    Coroutine *check_bound_co() {
        if (co) {
            openswoole_fatal_error(OSW_ERROR_CO_HAS_BEEN_BOUND, "cURL is executing, cannot be operated");
            return nullptr;
        }
