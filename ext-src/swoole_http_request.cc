...
static sw_inline const char *http_get_method_name(uint8_t method) {
...
const llhttp_settings_t *swoole_http_server_get_parser_settings() {
...
{
    PHP_ME(swoole_http_request, rawContent, arginfo_class_Swoole_Http_Request_rawContent, ZEND_ACC_PUBLIC)
    PHP_MALIAS(swoole_http_request, getContent, rawContent, arginfo_class_Swoole_Http_Request_getContent, ZEND_ACC_PUBLIC)
...
static PHP_METHOD(swoole_http_request, create) {
    ...
    *zrequest_object = *return_value;
    ...
    llhttp_init(parser, HTTP_REQUEST, swoole_http_server_get_parser_settings());
    parser->data = ctx;
