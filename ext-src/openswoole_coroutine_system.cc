...
#include "openswoole_coroutine_system_arginfo.h"
...
static zend_class_entry *openswoole_coroutine_system_ce;
...
static const zend_function_entry openswoole_coroutine_system_methods[] =
{
    ZEND_FENTRY(gethostbyname, ZEND_FN(openswoole_coroutine_gethostbyname), arginfo_class_OpenSwoole_Coroutine_System_gethostbyname, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, exec, arginfo_class_OpenSwoole_Coroutine_System_exec, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, sleep, arginfo_class_OpenSwoole_Coroutine_System_sleep, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, usleep, arginfo_class_OpenSwoole_Coroutine_System_usleep, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, getaddrinfo, arginfo_class_OpenSwoole_Coroutine_System_getaddrinfo, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, statvfs, arginfo_class_OpenSwoole_Coroutine_System_statvfs, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, readFile, arginfo_class_OpenSwoole_Coroutine_System_readFile, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, writeFile, arginfo_class_OpenSwoole_Coroutine_System_writeFile, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, wait, arginfo_class_OpenSwoole_Coroutine_System_wait, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, waitPid, arginfo_class_OpenSwoole_Coroutine_System_waitPid, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, waitSignal, arginfo_class_OpenSwoole_Coroutine_System_waitSignal, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, waitEvent, arginfo_class_OpenSwoole_Coroutine_System_waitEvent, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, dnsLookup, arginfo_class_OpenSwoole_Coroutine_System_dnsLookup, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_ME(openswoole_coroutine_system, clearDNSCache, arginfo_class_OpenSwoole_Coroutine_System_clearDNSCache, ZEND_ACC_PUBLIC | ZEND_ACC_STATIC)
    PHP_FE_END
};
...
void php_openswoole_coroutine_system_minit(int module_number) {
    OSW_INIT_CLASS_ENTRY_BASE(openswoole_coroutine_system,
                             "OpenSwoole\\Coroutine\\System",
                             nullptr,
                             nullptr,
                             openswoole_coroutine_system_methods,
                             nullptr);
    OSW_SET_CLASS_CREATE(openswoole_coroutine_system, osw_zend_create_object_deny);
}
...
PHP_METHOD(openswoole_coroutine_system, sleep) {
    ...
    if (UNEXPECTED(seconds < 0)) {
        php_openswoole_fatal_error(E_WARNING, "Number of seconds must be greater than or equal to " ZEND_TOSTR(0));
        RETURN_FALSE;
    }
...
PHP_METHOD(openswoole_coroutine_system, usleep) {
    ...
    if (UNEXPECTED(microseconds < 0)) {
        php_openswoole_fatal_error(E_WARNING, "Number of microseconds must be greater than or equal to " ZEND_TOSTR(0));
        RETURN_FALSE;
    }
...
static void co_socket_read(int fd, zend_long length, INTERNAL_FUNCTION_PARAMETERS) {
    ...
    Socket _socket(fd, OSW_SOCK_RAW);

    zend_string *buf = zend_string_alloc(length + 1, 0);
    size_t nbytes = length <= 0 ? OSW_BUFFER_SIZE_STD : length;
...
PHP_FUNCTION(openswoole_coroutine_gethostbyname) {
    ...
    if (l_domain_name == 0) {
        php_openswoole_fatal_error(E_WARNING, "domain name is empty");
        RETURN_FALSE;
    }

    if (family != AF_INET && family != AF_INET6) {
        php_openswoole_fatal_error(E_WARNING, "unknown protocol family, must be AF_INET or AF_INET6");
        RETURN_FALSE;
    }
...
PHP_METHOD(openswoole_coroutine_system, getaddrinfo) {
    ...
    if (l_hostname == 0) {
        php_openswoole_fatal_error(E_WARNING, "hostname is empty");
        RETURN_FALSE;
    }
...
PHP_METHOD(openswoole_coroutine_system, statvfs) {
    ...
    openswoole_coroutine_statvfs(path, &_stat);
...
PHP_METHOD(openswoole_coroutine_system, exec) {
    ...
    if (php_openswoole_signal_isset_handler(SIGCHLD)) {
        php_openswoole_error(E_WARNING, "The signal [SIGCHLD] is registered, cannot execute openswoole_coroutine_exec");
        RETURN_FALSE;
    }
    ...
    pid_t pid;
    int fd = openswoole_shell_exec(command, &pid, get_error_stream);
    if (fd < 0) {
        php_openswoole_error(E_WARNING, "Unable to execute '%s'", command);
        RETURN_FALSE;
    }

    String *buffer = new String(1024);
    Socket socket(fd, OSW_SOCK_UNIX_STREAM);
    while (1) {
    ...
    int status;
    pid_t _pid = openswoole_coroutine_waitpid(pid, &status, 0);
    if (_pid > 0) {
        array_init(return_value);
...
static void openswoole_coroutine_system_wait(INTERNAL_FUNCTION_PARAMETERS, pid_t pid, double timeout) {
    int status;
...
PHP_METHOD(openswoole_coroutine_system, wait) {
    ...
    openswoole_coroutine_system_wait(INTERNAL_FUNCTION_PARAM_PASSTHRU, -1, timeout);
}
...
PHP_METHOD(openswoole_coroutine_system, waitPid) {
    ...
    openswoole_coroutine_system_wait(INTERNAL_FUNCTION_PARAM_PASSTHRU, pid, timeout);
}
...
PHP_METHOD(openswoole_coroutine_system, waitSignal) {
    ...
    if (!System::wait_signal(signo, timeout)) {
        if (openswoole_get_last_error() == EBUSY) {
            php_openswoole_fatal_error(E_WARNING, "Unable to wait signal, async signal listener has been registered");
        } else if (openswoole_get_last_error() == EINVAL) {
            php_openswoole_fatal_error(E_WARNING, "Invalid signal [" ZEND_LONG_FMT "]", signo);
        }
...
PHP_METHOD(openswoole_coroutine_system, waitEvent) {
    ...
    int fd = php_openswoole_convert_to_fd(zfd);
    if (fd < 0) {
...
PHP_METHOD(openswoole_coroutine_system, dnsLookup) {
    ...
    zval *domain;
    long type = AF_INET;
    double timeout = openswoole::network::Socket::default_dns_timeout;
    if (zend_parse_parameters(ZEND_NUM_ARGS(), "z|dl", &domain, &timeout, &type) == FAILURE) {
        RETURN_FALSE;
    }

    if (Z_TYPE_P(domain) != IS_STRING) {
        php_openswoole_fatal_error(E_WARNING, "invalid domain name");
        RETURN_FALSE;
    }

    if (Z_STRLEN_P(domain) == 0) {
        php_openswoole_fatal_error(E_WARNING, "domain name empty");
        RETURN_FALSE;
    }
    ...
    vector<string> result = openswoole::coroutine::dns_lookup(Z_STRVAL_P(domain), type, timeout);
    if (result.empty()) {
    ...
    cache->update_time = Timer::get_absolute_msec() + (int64_t) (OpenSwooleG.dns_cache_refresh_time * 1000);
}
