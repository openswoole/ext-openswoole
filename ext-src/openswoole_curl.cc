...
Socket *Multi::create_socket(CURL *cp, curl_socket_t sockfd) {
    if (!openswoole_event_isset_handler(PHP_OPENSWOOLE_FD_CO_CURL)) {
        openswoole_event_set_handler(PHP_OPENSWOOLE_FD_CO_CURL | OSW_EVENT_READ, cb_readable);
        openswoole_event_set_handler(PHP_OPENSWOOLE_FD_CO_CURL | OSW_EVENT_WRITE, cb_writable);
        openswoole_event_set_handler(PHP_OPENSWOOLE_FD_CO_CURL | OSW_EVENT_ERROR, cb_error);
    }
    Socket *socket = new Socket();
    socket->fd = sockfd;
    socket->removed = 1;
    socket->fd_type = (FdType) PHP_OPENSWOOLE_FD_CO_CURL;
...
void Multi::del_event(CURL *cp, void *socket_ptr, curl_socket_t sockfd) {
    ...
    if (socket->events && openswoole_event_is_available() && openswoole_event_del(socket) == OSW_OK) {
        event_count_--;
    }
    socket->fd = -1;
    socket->free();
    ...
    openswoole_trace_log(OSW_TRACE_CO_CURL, OSW_ECHO_RED " handle=%p, curl=%p, fd=%d", "[DEL]", handle, cp, sockfd);
}
...
void Multi::set_event(CURL *cp, void *socket_ptr, curl_socket_t sockfd, int action) {
    ...
    socket->fd = sockfd;
    if (socket->events) {
        openswoole_event_set(socket, events);
    } else {
        if (openswoole_event_add(socket, events) == OSW_OK) {
            event_count_++;
        }
    }
    Handle *handle = get_handle(cp);
    ...
    openswoole_trace_log(
        OSW_TRACE_CO_CURL, OSW_ECHO_GREEN " handle=%p, curl=%p, fd=%d, events=%d", "[ADD]", handle, cp, sockfd, events);
}
...
CURLcode Multi::exec(php_curl *ch) {
    ...
    OSW_LOOP {
        if (handle->socket && handle->socket->removed && handle->socket->fd > 0) {
            if (openswoole_event_add(handle->socket, get_event(handle->action)) == OSW_OK) {
                event_count_++;
            }
            openswoole_trace_log(
                OSW_TRACE_CO_CURL, "resume, handle=%p, curl=%p, fd=%d", handle, ch->cp, handle->socket->get_fd());
        }
        ...
        if (sockfd >= 0) {
            ...
            if (handle->socket && !handle->socket->removed && openswoole_event_del(handle->socket) == OSW_OK) {
                event_count_--;
            }
        }
        del_timer();
        ...
        openswoole_trace_log(OSW_TRACE_CO_CURL,
...
CURLcode Multi::read_info() {
    ...
    while ((message = curl_multi_info_read(multi_handle_, &pending))) {
        ...
        default:
            openswoole_warning("CURLMSG default");
            break;
        }
    }
    return CURLE_OK;
...
long Multi::select(php_curlm *mh, double timeout) {
    ...
    for (zend_llist_element *element = mh->easyh.head; element; element = element->next) {
        ...
        php_curl *ch;
        if ((ch = openswoole_curl_get_handle(z_ch, false)) == NULL) {
            continue;
        }
        Handle *handle = get_handle(ch->cp);
    ...
    for (zend_llist_element *element = mh->easyh.head; element; element = element->next) {
        ...
        Handle *handle = get_handle(ch->cp);
        if (handle && handle->socket && !handle->socket->removed && openswoole_event_del(handle->socket) == OSW_OK) {
            openswoole_trace_log(
                OSW_TRACE_CO_CURL, "suspend, handle=%p, curl=%p, fd=%d", handle, ch->cp, handle->socket->get_fd());
            event_count_--;
        }
    }
    del_timer();
    ...
    if (selector->timer_callback) {
        ...
        openswoole_trace_log(OSW_TRACE_CO_CURL, "socket_action[timer], running_handles=%d", running_handles_);
    }
    ...
    for (auto iter = selector->active_handles.begin(); iter != selector->active_handles.end(); iter++) {
        ...
        openswoole_trace_log(OSW_TRACE_CO_CURL, "socket_action[socket], running_handles=%d", running_handles_);
    }
...
void Multi::callback(Handle *handle, int event_bitmask) {
    openswoole_trace_log(OSW_TRACE_CO_CURL, "handle=%p, event_bitmask=%d, co=%p", handle, event_bitmask, co);
    if (handle) {
    ...
    if (!co) {
        if (handle) {
            if (openswoole_event_del(handle->socket) == OSW_OK) {
                event_count_--;
            }
        } else {
            del_timer();
