...
PHP_METHOD(swoole_coroutine_system, sleep) {
    ...
    if (UNEXPECTED(seconds < SW_TIMER_MIN_SEC * 1000)) {
        php_swoole_fatal_error(E_WARNING, "Timer must be greater than or equal to " ZEND_TOSTR(SW_TIMER_MIN_SEC * 1000));
        RETURN_FALSE;
    }
...
PHP_METHOD(swoole_coroutine_system, usleep) {
    ...
    ZEND_PARSE_PARAMETERS_START(1, 1)
    Z_PARAM_DOUBLE(microseconds)
    ZEND_PARSE_PARAMETERS_END_EX(RETURN_FALSE);

    if (UNEXPECTED(microseconds < SW_TIMER_MIN_SEC * 1000000)) {
        php_swoole_fatal_error(E_WARNING, "Timer must be greater than or equal to " ZEND_TOSTR(SW_TIMER_MIN_SEC * 1000000));
        RETURN_FALSE;
    }
