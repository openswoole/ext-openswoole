...
void Socket::timer_callback(Timer *timer, TimerNode *tnode) {
    ...
    if (osw_likely(tnode == socket->read_timer)) {
...
int Socket::readable_event_callback(Reactor *reactor, Event *event) {
    ...
    if (osw_unlikely(socket->want_event != OSW_EVENT_NULL)) {
        if (socket->want_event == OSW_EVENT_READ) {
            socket->write_co->resume();
        }
    } else
#endif
...
#ifdef OSW_USE_OPENSSL
    if (osw_unlikely(socket->want_event != OSW_EVENT_NULL)) {
        if (socket->want_event == OSW_EVENT_WRITE) {
            socket->read_co->resume();
        }
    } else
#endif
...
bool Socket::add_event(const EventType event) {
    bool ret = true;
    if (osw_likely(!(socket->events & event))) {
        if (socket->removed) {
            ret = openswoole_event_add(socket, event) == OSW_OK;
        } else {
            ret = openswoole_event_set(socket, socket->events | event) == OSW_OK;
        }
    }
    set_err(ret ? 0 : errno);
    return ret;
...
bool Socket::wait_event(const EventType event, const void **__buf, size_t __n) {
    ...
    set_err(0);
#ifdef OSW_USE_OPENSSL
    if (osw_unlikely(socket->ssl && ((event == OSW_EVENT_READ && socket->ssl_want_write) ||
                                    (event == OSW_EVENT_WRITE && socket->ssl_want_read)))) {
        if (osw_likely(socket->ssl_want_write && add_event(OSW_EVENT_WRITE))) {
            want_event = OSW_EVENT_WRITE;
        } else if (socket->ssl_want_read && add_event(OSW_EVENT_READ)) {
            want_event = OSW_EVENT_READ;
        } else {
            return false;
        }
...
#endif
        if (osw_unlikely(!add_event(event))) {
        return false;
    }
    openswoole_trace_log(OSW_TRACE_SOCKET,
                     ...
                     :
#endif
                     event == OSW_EVENT_READ ? "READ"
                                            : "WRITE");

    Coroutine::CancelFunc cancel_fn = [this, event](Coroutine *co) { return cancel(event); };

    if (osw_likely(event == OSW_EVENT_READ)) {
        read_co = co;
        read_co->yield(&cancel_fn);
        read_co = nullptr;
    } else if (event == OSW_EVENT_WRITE) {
        if (osw_unlikely(!zero_copy && __n > 0 && *__buf != get_write_buffer()->str)) {
            write_buffer->clear();
            if (write_buffer->append((const char *) *__buf, __n) != OSW_OK) {
                set_err(ENOMEM);
                goto _failed;
            }
...
_failed:
    ...
    if (osw_likely(want_event == OSW_EVENT_NULL || !has_bound()))
#endif
    {
        Reactor *reactor = OpenSwooleTG.reactor;
        if (osw_likely(added_event == OSW_EVENT_READ)) {
    ...
    openswoole_trace_log(OSW_TRACE_SOCKET,
                     ...
                     : added_event == OSW_EVENT_READ ? "READ"
                                                    : "WRITE");
...
bool Socket::socks5_handshake() {
    ...
    socks5_proxy->state = OSW_SOCKS5_STATE_HANDSHAKE;
    if (send(ctx->buf, 3) != 3) {
        return false;
    }
...
#define HTTP_PROXY_FMT                                                                                                 \
    ...
    "Host: %.*s:%d\r\n"                                                                                                \
    "User-Agent: OpenSwoole/" OPENSWOOLE_VERSION "\r\n"                                                                    \
    ...
    if (!http_proxy->password.empty()) {
        ...
        n = osw_snprintf(send_buffer->str,
                        send_buffer->size,
    ...
    openswoole_trace_log(OSW_TRACE_HTTP_CLIENT, "proxy request: <<EOF\n%.*sEOF", n, send_buffer->str);
    ...
    memcpy(protocol.package_eof, OSW_STRS("\r\n\r\n"));
    ...
    openswoole_trace_log(OSW_TRACE_HTTP_CLIENT, "proxy response: <<EOF\n%.*sEOF", n, recv_buffer->str);
    ...
    if (!ret) {
        set_err(OSW_ERROR_HTTP_PROXY_BAD_RESPONSE,
...
bool Socket::init_sock() {
    socket = make_socket(type, OSW_FD_CORO_SOCKET, OSW_SOCK_CLOEXEC | OSW_SOCK_NONBLOCK);
    if (socket == nullptr) {
        return false;
    }
    sock_fd = socket->fd;
...
bool Socket::init_reactor_socket(int _fd) {
    socket = openswoole::make_socket(_fd, OSW_FD_CORO_SOCKET);
    sock_fd = _fd;
    socket->object = this;
...
Socket::Socket(int _fd, SocketType _type) {
    init_sock_type(_type);
    if (osw_unlikely(!init_reactor_socket(_fd))) {
        return;
    }
    socket->set_nonblock();
    init_options();
...
Socket::Socket(network::Socket *sock, Socket *server_sock) {
    ...
    socket->object = this;
    socket->socket_type = type;
    socket->fd_type = OSW_FD_CORO_SOCKET;
    init_options();
...
bool Socket::connect(const struct sockaddr *addr, socklen_t addrlen) {
    ...
    if (retval < 0) {
        if (errno != EINPROGRESS) {
            ...
            if (!timer.start() || !wait_event(OSW_EVENT_WRITE)) {
                if (closed) {
...
bool Socket::connect(std::string _host, int _port, int flags) {
        ...
        if (sock_domain == AF_INET) {
            ...
            if (!inet_pton(AF_INET, connect_host.c_str(), &socket->info.addr.inet_v4.sin_addr)) {
                ...
                if (connect_host.empty()) {
                    set_err(openswoole_get_last_error(), openswoole_strerror(openswoole_get_last_error()));
                    return false;
                }
                continue;
            } else {
            ...
            if (!inet_pton(AF_INET6, connect_host.c_str(), &socket->info.addr.inet_v6.sin6_addr)) {
                ...
                if (connect_host.empty()) {
                    set_err(openswoole_get_last_error());
                    return false;
                }
                continue;
            } else {
    ...
    if (socks5_proxy && socks5_handshake() == false) {
        if (errCode == 0) {
            set_err(OSW_ERROR_SOCKS5_HANDSHAKE_FAILED);
        }
        return false;
    }
    // http proxy
    if (http_proxy && !http_proxy->dont_handshake && http_proxy_handshake() == false) {
        if (errCode == 0) {
            set_err(OSW_ERROR_HTTP_PROXY_HANDSHAKE_FAILED);
        }
        return false;
    }
#ifdef OSW_USE_OPENSSL
    ...
    if (ssl_context) {
        if (!ssl_handshake()) {
            if (errCode == 0) {
                set_err(OSW_ERROR_SSL_HANDSHAKE_FAILED);
            }
            return false;
        }
    }
...
bool Socket::poll(EventType type) {
    if (osw_unlikely(!is_available(type))) {
        return -1;
    }
...
ssize_t Socket::recv(void *__buf, size_t __n) {
    if (osw_unlikely(!is_available(OSW_EVENT_READ))) {
        return -1;
    }
    ssize_t retval;
    TimerController timer(&read_timer, read_timeout, this, timer_callback);
    ...
    } while (retval < 0 && socket->catch_error(errno) == OSW_WAIT && timer.start() && wait_event(OSW_EVENT_READ));
    check_return_value(retval);
    return retval;
}

ssize_t Socket::send(const void *__buf, size_t __n) {
    if (osw_unlikely(!is_available(OSW_EVENT_WRITE))) {
        return -1;
    }
    ssize_t retval;
    ...
    } while (retval < 0 && socket->catch_error(errno) == OSW_WAIT && timer.start() &&
             wait_event(OSW_EVENT_WRITE, &__buf, __n));
    check_return_value(retval);
    return retval;
}
...
ssize_t Socket::recv_with_buffer(void *__buf, size_t __n) {
    ...
    size_t copy_bytes = OSW_MIN(__n - buffer_bytes, buffer->length - buffer->offset);
...
ssize_t Socket::readv_all(network::IOVector *io_vector) {
    ...
    retval = socket->readv(io_vector);
    openswoole_trace_log(OSW_TRACE_SOCKET, "readv %ld bytes, errno=%d", retval, errno);

    if (retval < 0 && socket->catch_error(errno) != OSW_WAIT) {
        set_err(errno);
        return retval;
    }
    ...
    EventBarrier barrier = [&io_vector, &total_bytes, &retval, this]() -> bool {
        ...
        return retval < 0 && socket->catch_error(errno) == OSW_WAIT;
    };

    recv_barrier = &barrier;
    if (timer.start() && wait_event(OSW_EVENT_READ)) {
        check_return_value(retval);
    }
...
ssize_t Socket::writev(network::IOVector *io_vector) {
    ...
    } while (retval < 0 && socket->catch_error(errno) == OSW_WAIT && timer.start() && wait_event(OSW_EVENT_WRITE));
    check_return_value(retval);
...
ssize_t Socket::writev_all(network::IOVector *io_vector) {
    ...
    retval = socket->writev(io_vector);
    openswoole_trace_log(OSW_TRACE_SOCKET, "writev %ld bytes, errno=%d", retval, errno);
...
ssize_t Socket::recv_all(void *__buf, size_t __n) {
    ...
    EventBarrier barrier = [&__n, &total_bytes, &retval, &__buf, this]() -> bool {
        ...
        return (retval < 0 && socket->catch_error(errno) == OSW_WAIT) || (retval > 0 && (total_bytes += retval) < __n);
    };
...
bool Socket::bind(std::string address, int port) {
    if (osw_unlikely(!is_available(OSW_EVENT_NULL))) {
        return false;
    }
...
bool Socket::listen(int backlog) {
    ...
    this->backlog = backlog <= 0 ? OSW_BACKLOG : backlog;
...
Socket *Socket::accept(double timeout) {
    ...
    if (osw_unlikely(client_sock->get_fd() < 0)) {
        openswoole_sys_warning("new Socket() failed");
        set_err(errno);
        delete client_sock;
        return nullptr;
    }
...
bool Socket::ssl_handshake() {
    ...
    if (!ssl_is_server) {
        while (true) {
            ...
            if (socket->ssl_state == OSW_SSL_STATE_WAIT_STREAM) {
                TimerController timer(&read_timer, read_timeout, this, timer_callback);
                if (!timer.start() || !wait_event(OSW_EVENT_READ)) {
                    return false;
                }
            } else if (socket->ssl_state == OSW_SSL_STATE_READY) {
                break;
            }
        }
    } else {
        ...
        } while (retval == OSW_WAIT && timer.start() && wait_event(OSW_EVENT_READ));

        if (retval != OSW_READY) {
            set_err(OSW_ERROR_SSL_BAD_CLIENT);
            return false;
        }
    }
...
bool Socket::ssl_verify(bool allow_self_signed) {
    if (!socket->ssl_verify(allow_self_signed)) {
        set_err(OSW_ERROR_SSL_VERIFY_FAILED);
        return false;
    }
#ifdef SSL_CTRL_SET_TLSEXT_HOSTNAME
...
std::string Socket::ssl_get_peer_cert() {
    if (!socket->ssl_get_peer_certificate(osw_tg_buffer())) {
        return "";
    } else {
        return osw_tg_buffer()->to_std_string();
    }
}
#endif
...
#endif
        {
            n = ::openswoole_sendfile(sock_fd, file.get_fd(), &offset, sendn);
        }
        if (n > 0) {
            continue;
        } else if (n == 0) {
            set_err(OSW_ERROR_SYSTEM_CALL_FAIL, "sendfile return zero");
            return false;
...
ssize_t Socket::sendto(const std::string &host, int port, const void *__buf, size_t __n) {
    ...
    for (size_t i = 0; i < 2; i++) {
        ...
        } else if (type == OSW_SOCK_UNIX_DGRAM) {
            addr.un.sun_family = AF_UNIX;
            openswoole_strlcpy(addr.un.sun_path, host.c_str(), sizeof(addr.un.sun_path));
    ...
    if (addr_size > 0) {
        ...
        do {
            ...
            openswoole_trace_log(OSW_TRACE_SOCKET, "sendto %ld/%ld bytes, errno=%d", retval, __n, errno);
        } while (retval < 0 && (errno == EINTR || (socket->catch_error(errno) == OSW_WAIT && timer.start() &&
                                                   wait_event(OSW_EVENT_WRITE, &__buf, __n))));
        check_return_value(retval);
    }
...
ssize_t Socket::recvfrom(void *__buf, size_t __n, struct sockaddr *_addr, socklen_t *_socklen) {
    ...
    do {
        ...
        openswoole_trace_log(OSW_TRACE_SOCKET, "recvfrom %ld/%ld bytes, errno=%d", retval, __n, errno);
    } while (retval < 0 && ((errno == EINTR) ||
                            (socket->catch_error(errno) == OSW_WAIT && timer.start() && wait_event(OSW_EVENT_READ))));
    check_return_value(retval);
    return retval;
}

ssize_t Socket::recv_packet_with_length_protocol() {
    ssize_t packet_len = OSW_BUFFER_SIZE_STD;
    ssize_t retval;
...
_get_length:
    ...
    openswoole_trace_log(OSW_TRACE_SOCKET, "packet_len=%ld, length=%ld", packet_len, read_buffer->length);
    if (packet_len < 0) {
        set_err(OSW_ERROR_PACKAGE_LENGTH_NOT_FOUND, "get package length failed");
        return 0;
        ...
        openswoole_error_log(OSW_LOG_WARNING,
                         ...
                         packet_len);
        set_err(OSW_ERROR_PACKAGE_LENGTH_TOO_LARGE, osw_error);
        return -1;
    }
...
ssize_t Socket::recv_packet_with_eof_protocol() {
    ...
    while (1) {
        ...
        if (l_buf > OSW_BUFFER_SIZE_BIG) {
            l_buf = OSW_BUFFER_SIZE_BIG;
        }
    ...
    _find_eof:
        eof = openswoole_strnpos(read_buffer->str, read_buffer->length, protocol.package_eof, protocol.package_eof_len);
        if (eof >= 0) {
            return (read_buffer->offset = eof + protocol.package_eof_len);
        }
        if (read_buffer->length == protocol.package_max_length) {
            read_buffer->clear();
            set_err(OSW_ERROR_PACKAGE_LENGTH_TOO_LARGE, "no package eof, package_max_length exceeded");
            return -1;
        }
...
Socket::~Socket() {
#ifdef OSW_DEBUG
    if (OpenSwooleG.running) {
        OSW_ASSERT(!has_bound() && socket->removed);
    }
#endif
    if (read_buffer) {
        delete read_buffer;
...
std::string HttpProxy::get_auth_str() {
    char auth_buf[256];
    char encode_buf[512];
    size_t n = osw_snprintf(auth_buf,
                           sizeof(auth_buf),
                           "%.*s:%.*s",
                           (int) username.length(),
                           username.c_str(),
