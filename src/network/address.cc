...
bool Address::assign(SocketType _type, const std::string &_host, int _port) {
    ...
    if (_type == OSW_SOCK_TCP || _type == OSW_SOCK_UDP) {
    ...
    } else if (_type == OSW_SOCK_TCP6 || _type == OSW_SOCK_UDP6) {
    ...
    } else if (_type == OSW_SOCK_UNIX_STREAM || _type == OSW_SOCK_UNIX_DGRAM) {
        addr.un.sun_family = AF_UNIX;
        openswoole_strlcpy(addr.un.sun_path, host, sizeof(addr.un.sun_path));
