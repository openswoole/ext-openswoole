...
void Manager::kill_timeout_process(Timer *timer, TimerNode *tnode) {
    ...
    for (auto i = _list->begin(); i != _list->end(); i++) {
        ...
        if (openswoole_kill(pid, 0) == -1) {
            continue;
        }
        if (openswoole_kill(pid, SIGKILL) < 0) {
            openswoole_sys_warning("kill(%d, SIGKILL) [%u] failed", pid, worker_id);
        } else {
...
void Manager::add_timeout_killer(Worker *workers, int n) {
     ...
     */
    openswoole_timer_after((long) (server_->max_wait_time * 2 * 1000), kill_timeout_process, _list);
}
...
int Server::start_manager_process() {
    ...
    case 0: {
        ...
        OSW_START_SLEEP;
        if (!is_started()) {
            openswoole_error("master process is not running");
            return OSW_ERR;
        }
        ...
        if (task_worker_num > 0) {
            if (gs->task_workers.start() == OSW_ERR) {
                openswoole_error("failed to start task workers");
                return OSW_ERR;
            }
        }
        ...
        if (!user_worker_list.empty()) {
            for (auto worker : user_worker_list) {
                ...
                if (pid < 0) {
                    openswoole_error("failed to start user workers");
                    return OSW_ERR;
                }
            }
        }
...
void Server::check_worker_exit_status(int worker_id, const ExitStatus &exit_status) {
    if (exit_status.get_status() != 0) {
        openswoole_warning("worker(pid=%d, id=%d) abnormal exit, status=%d, signal=%d"
                       "%s",
                       exit_status.get_pid(),
                       worker_id,
...
void Manager::start(Server *_server) {
    ...
    openswoole_signal_set(SIGTERM, signal_handler);
    openswoole_signal_set(SIGUSR1, signal_handler);
    openswoole_signal_set(SIGUSR2, signal_handler);
    openswoole_signal_set(SIGIO, signal_handler);
    openswoole_signal_set(SIGALRM, signal_handler);
#ifdef SIGRTMIN
    openswoole_signal_set(SIGRTMIN, signal_handler);
#endif
    ...
    if (_server->manager_alarm > 0) {
        openswoole_timer_add((long) (_server->manager_alarm * 1000), true, timer_callback, _server);
    }
        ...
        if (exit_status.get_pid() < 0) {
            ...
            _error:
                if (errno > 0 && errno != EINTR) {
                    openswoole_sys_warning("wait() failed");
                }
                continue;
            }
            // reload task & event workers
            else if (reload_all_worker) {
                openswoole_info("Server is reloading all workers");
                ...
                if (!reload_init) {
                    ...
                    if (_server->reload_async) {
                        OSW_LOOP_N(_server->worker_num) {
                            if (openswoole_kill(reload_workers[i].pid, SIGTERM) < 0) {
                                openswoole_sys_warning("swKill(%d, SIGTERM) [%d] failed", reload_workers[i].pid, i);
                            }
                        }
                ...
                openswoole_info("Server has done all workers reloading");
                //
                goto _kill_worker;
            }
            // only reload task workers
                ...
                if (_server->task_worker_num == 0) {
                    openswoole_warning("cannot reload task workers, task workers is not started");
                    reloading = false;
                    continue;
                }
                openswoole_info("Server is reloading task workers now");
                if (_server->onBeforeReload != nullptr) {
                    _server->onBeforeReload(_server);
                }
                if (!reload_init) {
    ...
    _kill_worker:
        if (reloading) {
            ...
            osw_atomic_fetch_add(&_server->gs->reload_count, 1);
            // reload finish
            ...
            if (openswoole_kill(reload_worker_pid, SIGTERM) < 0) {
    ...
    if (_server->max_wait_time) {
        ...
        if (_server->task_worker_num > 0) {
            OSW_LOOP_N(_server->gs->task_workers.worker_num) {
...
/**
 * @return: success returns pid, failure returns OSW_ERR.
 */
    ...
    do {
        if (serv->gs->task_workers.map_) {
            ...
            if (iter != serv->gs->task_workers.map_->end()) {
                worker_type = OSW_PROCESS_TASKWORKER;
        ...
        if (!serv->user_worker_map.empty()) {
            ...
            if (iter != serv->user_worker_map.end()) {
                worker_type = OSW_PROCESS_USERWORKER;
                exit_worker = iter->second;
                break;
            }
        }
...
void Server::kill_user_workers() {
    ...
    for (auto &kv : user_worker_map) {
        openswoole_kill(kv.second->pid, SIGTERM);
    }

    for (auto &kv : user_worker_map) {
        int __stat_loc;
        if (openswoole_waitpid(kv.second->pid, &__stat_loc, 0) < 0) {
            openswoole_sys_warning("waitpid(%d) failed", kv.second->pid);
        }
    }
}
...
void Server::kill_event_workers() {
    ...
    OSW_LOOP_N(worker_num) {
        openswoole_trace("[Manager]kill worker processor");
        openswoole_kill(workers[i].pid, SIGTERM);
    }
    OSW_LOOP_N(worker_num) {
        if (openswoole_waitpid(workers[i].pid, &status, 0) < 0) {
            openswoole_sys_warning("waitpid(%d) failed", workers[i].pid);
        }
    }
}
...
pid_t Server::spawn_event_worker(Worker *worker) {
    ...
    // fork() failed
    if (pid < 0) {
        openswoole_sys_warning("Fork Worker failed");
        return OSW_ERR;
    }
...
pid_t Server::spawn_user_worker(Worker *worker) {
    ...
    // child
        ...
        OpenSwooleG.process_type = OSW_PROCESS_USERWORKER;
